  ...
  return (
    <PageTransition>
      <div className="max-w-3xl mx-auto px-6 pt-28 pb-20">
        ...
        <FadeIn delay={experiences.length * 0.08}>
            ...
            <div className="space-y-6">
              {Object.entries(skills).map(([category, items]) => (
                <div key={category}>
                  ...
                  <StaggerContainer delay={experiences.length * 0.08} className="flex flex-wrap gap-2">
                    {items.map((skill) => (
                      <StaggerItem
                        key={skill}
                        className="text-xs px-2.5 py-1 rounded-full bg-border/50 text-muted border border-transparent hover:border-accent hover:text-accent transition-colors cursor-default inline-block"
                      >
                        {skill}
                      </StaggerItem>
                    ))}
